...
import numpy as np
import random
from transformers import LlamaForCausalLM, LlamaTokenizer, LlamaConfig, AutoTokenizer, AutoConfig, AutoModel
...
def init_weight_KG(model, embedding_path, config):
    for idx, layer in enumerate(model):
        if idx + 1 in list(config.layer_insertion):
            layer.init()
    for idx, layer in enumerate(model):
        if idx + 1 in list(config.layer_insertion):
            layer.KG_infuded_module.init(embedding_path)
    return model
...
def train(
    ...
    #embedding_path: str = "/data1/xdluo/alpaca-lora-main/data/kgs/wn_concept2vec.txt",
    #embedding_path: str = "/data1/xdluo/Knower/output/wi_ent_embeds.npy",
    #embedding_path: str = "/data1/xdluo/Knower/output/cn_rotate_ent_embeds.npy",
    #kg_path : str = "/data1/xdluo/alpaca-lora-main/data/kgs/wikidata/wiki_entity_name.txt",
    #kg_path : str = "/data1/xdluo/alpaca-lora-mains/data/kgs/",
    # training hyperparams
    batch_size: int =128,
    kg_path: str = "./data/kgs/conceptnet/concept.txt",
    #kg_path: str = "/data1/xdluo/alpaca-lora-main/data/kgs/conceptnet/ent_name.txt",
    ...
    model.base_model.model.model.layers = init_weight_KG(model.base_model.model.model.layers, embedding_path, model.config)
    print(model)
    model.activate_KG_modules()
    ...
    save_KG_module(model.base_model.model.model.layers, output_dir, model.config)
    print(
        "\n If there's a warning about missing keys above, please disregard :)"
    )


def save_KG_module(model, path, config):
    # print(model.state_dict())
    for idx, layer in enumerate(model):
        if idx + 1 in config.layer_insertion:
            tmp = path + r"/KG_retrieve_{}.bin".format(idx)
            torch.save(layer.KG_infuded_module.state_dict(), tmp)
    print("KG_retriever has been saved!")
